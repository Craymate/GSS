...
#Age can be plotted without means, because ages are more consistent. Other variables
#would produce error when plotting smooth line.

#So we plotted these two plots by using mean for each group.
...
res[,1] <- signif(quantile(mod$residuals), 4)
colnames(res) <- c("Value of Regression Residuals")

coe <- data.frame(
  row.names = paste(c("(Intercept)", "age", "income", "number of children"))) 
coe[,1] <- signif(mod$coefficients, 4)
colnames(coe) <- c("Regression Coefficients")
...
- Rohan Alexander and Sam Caetano. Data Preparation:
...
grid.arrange(tableGrob(coe), ncol=1, top = "Table 2: Regression Coefficients")
grid.arrange(tableGrob(summ), tableGrob(res), ncol=1, top = "Table 3: Regression Statistics")
