...
raw_data <- read_csv("./Input/AAQT1d4g.csv")
dict <- read_lines("./Input/gss_dict.txt", skip = 18) # skip is because of preamble content
# Now we need the labels because these are the actual responses that we need
labels_raw <- read_file("./Input/gss_labels.txt")
...
gss <- gss %>%
  select(caseid, age, income_respondent, feelings_life, total_children)


mut_inc <- function(x){
  for(i in 1:length(x)){
    if (x[i] == "$25,000 to $49,999"){
      x[i] <- (25000 + 50000)/2
    };
    if (x[i] == "Less than $25,000"){
      x[i] <- 25000/2
    };
    if (x[i] == "$50,000 to $74,999"){
      x[i] <- (50000 + 75000)/2
    };
    if (x[i] == "$75,000 to $99,999"){
      x[i] <- (75000 + 100000)/2
    };
    if (x[i] == "$100,000 to $ 124,999"){
      x[i] <- (100000 + 125000)/2
    };
    if (x[i] == "$125,000 and more"){
      x[i] <- 125000
    }
  }
  return(x)
...
gss <- gss %>%
  mutate(income_respondent = mut_inc(income_respondent))


write_csv(gss, "./Output/gss.csv")
